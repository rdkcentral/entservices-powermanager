...
namespace Plugin {
    ...
    Core::hresult PowerManagerImplementation::SetPowerState(const int keyCode, const PowerState newState, const string& reason)
    {
        ...
        if (currState != newState) {
            ...
            t2_event_d((char*)"SYST_INFO_POWER_CHANGE_split", telemetryPwrChange);
            ...
            if (POWER_STATE_STANDBY_DEEP_SLEEP == currState) {
                if (_deepSleepController.IsDeepSleepInProgress()
                    ...
                    snprintf(telemetryMsg, sizeof(telemetryMsg), "Ignore Power Mode Change to %s as device is in transient deep sleep state, elapsed: %" PRId64 " sec",
                             util::str(newState), std::chrono::duration_cast<std::chrono::seconds>(_deepSleepController.Elapsed()).count());
                    t2_event_d((char*)"SYST_ERR_SetPwrStateFail_split", telemetryMsg);
    ...
    Core::hresult PowerManagerImplementation::SetNetworkStandbyMode(const bool standbyMode)
    {
        ...
        t2_event_d((char*)"SYS_INFO_STANDBYMODE_split", telemetryMsg);
    ...
    void PowerManagerImplementation::onDeepSleepUserWakeup(const bool userWakeup)
    {
        ...
        t2_event_d((char*)"SYST_INFO_DS_WakeUp_split", (char*)"Resumed due to user action. Sending KED_DEEPSLEEP_WAKEUP.");
    ...
    void PowerManagerImplementation::onDeepSleepFailed()
    {
        ...
        t2_event_d((char*)"SYST_ERR_DSModeFail_split", (char*)"Failed to enter DeepSleep. Moving to On");
