...
#define POWER_MODE_CHANGE_ACK_TIMEOUT_SEC 10
#endif
...
namespace Plugin {
    ...
    Core::hresult PowerManagerImplementation::ScheduleDeepSleepWakeup(const long unixTime, const string& requestorId)
    {
        ...
        if (!requestorId.empty())
        {
            ...
            // Validate unixTime is positive, in range, and in the future
            time_t now = time(nullptr);
            if (unixTime <= 0 || unixTime > static_cast<long>(UINT32_MAX) || unixTime <= now) {
                LOGERR("unixTime %ld is invalid or not in future (now: %ld)", unixTime, now);
                _apiLock.Unlock();
                LOGINFO("<< errorCode: %u", errorCode);
                return errorCode;
            }
    ...
    void PowerManagerImplementation::startPowerModeChangeAcknowledgement(const int keyCode, PowerState currentState, PowerState newState, const std::string& reason)
    {
        ...
        ackController->Schedule(timeOut * 1000,
            [this, keyCode, currentState, newState, reason, ackController](bool isTimedout, bool isAborted) mutable {
                ...
                if (!isAborted) {
                    if (isTimedout) {
                        // Logging full diagnostic details here and then proceed with the power state change.
                        logUnresponsiveAckClients(ackController);
                    }
    ...
    void PowerManagerImplementation::logUnresponsiveAckClients(const std::shared_ptr<PreModeChangeController>& ackController)
    {
        _apiLock.Lock();

        const auto& pendingClientIds = ackController->Pending();
        LOGERR("PowerModeChangeAcknowledgement TIMEOUT after %d sec: %d client(s) failed to respond for transactionId: %d, "
               "targetPowerState: %s",
            POWER_MODE_CHANGE_ACK_TIMEOUT_SEC, int(pendingClientIds.size()), ackController->TransactionId(),
            util::str(ackController->powerState()));

        for (const auto& clientId : pendingClientIds) {
            const auto it          = _modeChangeAckClients.find(clientId);
            const std::string name = (it != _modeChangeAckClients.cend()) ? it->second : "<unknown/already-removed>";
            LOGERR("  -> unresponsive ack client: name='%s', acknowledgeClientId: %u, transactionId: %d",
                name.c_str(), clientId, ackController->TransactionId());
        }
    ...
    Core::hresult PowerManagerImplementation::PowerModeChangeAcknowledgement(const uint32_t acknowledgeClientId, const int transactionId)
    {
        ...
        if (_modeChangeAckController) {
            ...
            if (errorCode != Core::ERROR_NONE) {
                LOGWARN("PowerModeChangeAcknowledgement rejected - acknowledgeClientId: %u, transactionId: %d does "
                        "not match the active acknowledgement round (active transactionId: %d). This should not "
                        "happen and signifies a client-side issue.",
    ...
    void PowerManagerImplementation::onDeepSleepTimerWakeup(const int wakeupTimeout)
    {
        ...
        WakeupScheduleRegister::NearestWakeupSchedule* schedule = _wakeupScheduleRegister.getMostRecentlyExpiredWakeupSchedule();
