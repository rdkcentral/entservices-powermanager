...
*
* Copyright 2026 Liberty Global Service B.V.
*
...
#ifndef WAKEUP_SCHEDULE_REGISTER_H
...
class WakeupScheduleRegister
{
  public:
    ...
    WakeupScheduleRegister() = default;
    ~WakeupScheduleRegister()
    {
        clearSchedules(wakeupSchedules);
        clearSchedules(pastWakeupSchedules);
    }

    WakeupScheduleRegister(const WakeupScheduleRegister&) = delete;
    WakeupScheduleRegister& operator=(const WakeupScheduleRegister&) = delete;
    WakeupScheduleRegister(WakeupScheduleRegister&&) = delete;
    WakeupScheduleRegister& operator=(WakeupScheduleRegister&&) = delete;
    ...
    std::unique_ptr<NearestWakeupSchedule> peekNearestWakeupSchedule()
    {
        std::lock_guard<std::mutex> lock(_mutex);
        return peekNearestWakeupScheduleUnsafe();
    }

  private:
    std::unique_ptr<NearestWakeupSchedule> peekNearestWakeupScheduleUnsafe()
    {
        ...
        if (!wakeupSchedules.empty() && !wakeupSchedules[0]->powerStates.empty())
        {
            ...
            {
                std::vector<PowerState>::iterator it = std::find(wakeupSchedules[0]->powerStates.begin(),
                    wakeupSchedules[0]->powerStates.end(), priority[i]);

                if (it != wakeupSchedules[0]->powerStates.end())
                {
                    result = std::unique_ptr<NearestWakeupSchedule>(new NearestWakeupSchedule());
  ...
  public:
    ...
    std::unique_ptr<NearestWakeupSchedule> getNearestWakeupSchedule()
    {
        std::lock_guard<std::mutex> lock(_mutex);
    ...
    /* Moves any schedule(s) whose unixTime has already elapsed from the future list into the
       past list (same expiry rule as removeExpiredSchedules()), and returns an aggregated view
       of the schedule that most recently expired (i.e. the one that caused the current wakeup),
       or NULL if none expired. This is distinct from getNearestWakeupSchedule(), which only
       ever looks at the future list and therefore can never observe a schedule that just fired,
       since removeExpiredSchedules() has, by definition, already moved it out of that list. */
    std::unique_ptr<NearestWakeupSchedule> getMostRecentlyExpiredWakeupSchedule()
    {
        std::lock_guard<std::mutex> lock(_mutex);
        std::unique_ptr<NearestWakeupSchedule> result;
        ...
        {
            expiredSchedule = *wakeupSchedules.begin();
            pastWakeupSchedules.push_back(expiredSchedule);
            if (pastWakeupSchedules.size() > kMaxPastWakeupSchedules) {
                delete pastWakeupSchedules.front();
                pastWakeupSchedules.erase(pastWakeupSchedules.begin());
            }
            wakeupSchedules.erase(wakeupSchedules.begin());
        }

        if (expiredSchedule && !expiredSchedule->powerStates.empty())
        {
            const PowerState priority[] = {Operational, ActiveStandby};

            for (unsigned int i = 0; i < sizeof(priority) / sizeof(priority[0]) ; i++)
            {
                std::vector<PowerState>::iterator it = std::find(expiredSchedule->powerStates.begin(),
                    expiredSchedule->powerStates.end(), priority[i]);

                if (it != expiredSchedule->powerStates.end())
                {
                    result = std::unique_ptr<NearestWakeupSchedule>(new NearestWakeupSchedule());

                    if (result)
                    {
                        std::vector<std::string>::const_iterator rIt = expiredSchedule->requestorIds.begin();
                        std::vector<PowerState>::const_iterator pIt = expiredSchedule->powerStates.begin();
                        result->unixTime = expiredSchedule->unixTime;
                        result->powerState = *it;

                        for (; rIt != expiredSchedule->requestorIds.end() && pIt != expiredSchedule->powerStates.end(); rIt++, pIt++)
                        {
                            if (*pIt == *it)
                            {
                                result->requestorIds.push_back(*rIt);
    ...
    OperationStatus loadWakeupSchedulesFromFile(const char* path)
    {
        ...
        if (status == Successful)
        {
            ...
            std::sort(schedules.begin(), schedules.end(), lessSchedule);
  ...
  private:
    ...
    std::string scheduleToString(const Schedule *schedule)
    {
        ...
        /*
            ...
            where:
                ...
                mode        - [OA]           (Operational or ActiveStandby respectively),
    ...
    bool parseUnixTime(const std::string& input, size_t& i, UnixTime& output)
    {
        ...
        */
        ...
        for (; i < input.size(); i++)
        {
            ...
            {
                // Accumulate in a wider type and explicitly bound-check against UnixTime's max
                // before narrowing, since unsigned wraparound on UnixTime itself is not reliably
                // detectable via a "value went down" comparison (it can also wrap to a larger value).
                const unsigned long long candidate =
                    (static_cast<unsigned long long>(unixTime) * 10ULL) +
                    static_cast<unsigned long long>(input[i] - '0');

                if (candidate > static_cast<unsigned long long>(std::numeric_limits<UnixTime>::max()))
                {
                    break;
                }

                unixTime = static_cast<UnixTime>(candidate);
            }
            else
            {
                if (input[i] == ':')
    ...
    bool parseRequestorId(const std::string& input, size_t& i, std::string& output)
    {
        ...
        */
        ...
        while (i < input.size())
        {
            if (std::isalnum(static_cast<unsigned char>(input[i])) || input[i] == '-' || input[i] == '_')
            {
                requestorId += input[i];
                i++;
            }
    ...
    Schedule* stringToSchedule(const std::string& input)
    {
        ...
        if (input.size() > 0 && input[0] == '{' && input[input.size() - 1] == '}')
        {
            ...
            if (!parseUnixTime(input, i, unixTime))
            {
                while (i < input.size() && input[i] != '}')
                {
                    ...
                    /* Defensive guard: a parser that reports success must always consume
                       at least one character, otherwise malformed input would loop forever. */
                    if (i <= previousIndex)
                    {
                        error = "no progress while parsing schedule";
                        break;
                    }
                }
            }
        ...
        else
        {
            ERROR_LOG("%s(): malformed schedule detected! schedule = '%s', error = '%s'\n",
                __FUNCTION__, input.c_str(), (error != NULL ? error : "unknown parse error"));
        }
