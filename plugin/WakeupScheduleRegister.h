...
class WakeupScheduleRegister
{
  ...
  private:
    NearestWakeupSchedule* peekNearestWakeupScheduleUnsafe()
    {
    ...
    NearestWakeupSchedule* getMostRecentlyExpiredWakeupSchedule()
    {
        std::lock_guard<std::mutex> lock(_mutex);
        NearestWakeupSchedule* result = NULL;
        ...
        if (expiredSchedule && !expiredSchedule->powerStates.empty())
        {
            ...
            for (unsigned int i = 0; i < sizeof(priority) / sizeof(priority[0]) ; i++)
            {
                std::vector<PowerState>::iterator it = find(expiredSchedule->powerStates.begin(),
