"""
/**
 * @file TCID17_DeepSleepFlow.py
 * @brief L2 PowerManager combination testcase.
 *
 * @testcase TCID17_DeepSleepFlow
...
from PowerManager_CombinationHelpers import note_partial, parse_power_state, parse_time_since_wakeup
...
def run_test():
    ...
    if not is_ok(on_resp):
        log_error("TCID17_DeepSleepFlow Failed ❌ (failed to set ON baseline)")
        return False
    ...
    if not is_ok(light_resp):
        log_error("TCID17_DeepSleepFlow Failed ❌ (failed to enter LIGHT_SLEEP)")
        return False
    ...
    if not is_ok(register_resp):
        log_error("TCID17_DeepSleepFlow Failed ❌ (event registration failed)")
        return False
    ...
    if not is_ok(timer_resp):
        log_error("TCID17_DeepSleepFlow Failed ❌ (failed to set deep-sleep timer)")
        return False
    ...
    if not is_ok(deep_resp):
        log_error("TCID17_DeepSleepFlow Failed ❌ (failed to enter DEEP_SLEEP)")
        return False
    ...
    if not isinstance(state, dict) or state.get("currentState") != "LIGHT_SLEEP":
        log_error("TCID17_DeepSleepFlow Failed ❌ (expected LIGHT_SLEEP after timeout)")
    ...
    if previous != "DEEP_SLEEP":
        log_error("TCID17_DeepSleepFlow Failed ❌ (previousState mismatch after wake)")
        send_curl_command(PowerManagerApis.set_power_state("ON", standby_reason="PM-PLUGIN-001-restore"))
        return False
    ...
    if wake_age != 0:
        log_error("TCID17_DeepSleepFlow Failed ❌ (wake age should be 0 in LIGHT_SLEEP)")
        send_curl_command(PowerManagerApis.set_power_state("ON", standby_reason="PM-PLUGIN-001-restore"))
        return False
    ...
    if not is_ok(restore_resp):
        log_error("TCID17_DeepSleepFlow Failed ❌ (failed to restore ON)")
        return False
    ...
    if not isinstance(on_state, dict) or on_state.get("currentState") != "ON":
        log_error("TCID17_DeepSleepFlow Failed ❌ (expected ON after restore)")
        return False
    ...
    if not isinstance(on_wake_age, int) or on_wake_age <= 0:
        log_error("TCID17_DeepSleepFlow Failed ❌ (expected non-zero wake age in ON)")
        return False

    elapsed_time = time.perf_counter() - start_time
    msg = "TCID17_DeepSleepFlow Passed ✅"
