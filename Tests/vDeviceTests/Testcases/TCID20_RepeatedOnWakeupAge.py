"""
/**
 * @file TCID20_RepeatedOnWakeupAge.py
 * @brief L2 PowerManager combination testcase.
 *
 * @testcase TCID20_RepeatedOnWakeupAge
...
def run_test():
    ...
    if not is_ok(on_resp):
        log_error("TCID20_RepeatedOnWakeupAge Failed ❌ (failed to set ON baseline)")
        return False
    ...
    if not isinstance(first_value, int):
        log_error("TCID20_RepeatedOnWakeupAge Failed ❌ (invalid first wakeup age)")
        return False
    ...
    if not is_ok(repeat_resp):
        log_error("TCID20_RepeatedOnWakeupAge Failed ❌ (repeated ON request failed)")
        return False
    ...
    if not isinstance(second_value, int) or second_value < first_value:
        log_error("TCID20_RepeatedOnWakeupAge Failed ❌ (wakeup age reset after repeated ON)")
        return False

    elapsed_time = time.perf_counter() - start_time
    msg = "TCID20_RepeatedOnWakeupAge Passed ✅"
