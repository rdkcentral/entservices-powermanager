"""
/**
 * @file TCID12_WLANExternalWake.py
 * @brief L3 PowerManager combination testcase.
 *
 * @testcase TCID12_WLANExternalWake
...
def run_test():
    ...
    if not isinstance(original_mode, bool):
        log_error("TCID12_WLANExternalWake Failed ❌ (unable to read baseline network standby mode)")
        return False
    ...
    if not isinstance(original_config, list):
        log_error("TCID12_WLANExternalWake Failed ❌ (unable to read baseline config)")
        return False
    ...
    try:
        ...
        if not is_ok(enable_resp):
            log_error("TCID12_WLANExternalWake Failed ❌ (failed to enable network standby)")
            return False
        ...
        if not is_ok(timer_enable_resp):
            log_error("TCID12_WLANExternalWake Failed ❌ (failed to enable timer-backed network wake configuration)")
            return False
        ...
        if not isinstance(config, list):
            log_error("TCID12_WLANExternalWake Failed ❌ (invalid wakeup config)")
            return False
        if get_source_enabled(config, "WIFI") is not True or get_source_enabled(config, "LAN") is not True:
            log_error("TCID12_WLANExternalWake Failed ❌ (WIFI/LAN not enabled before network wake test)")
            return False
        if get_source_enabled(config, "TIMER") is not True:
            log_error("TCID12_WLANExternalWake Failed ❌ (TIMER not enabled before deep sleep)")
            return False
        ...
        if not is_ok(timer_resp):
            log_error("TCID12_WLANExternalWake Failed ❌ (failed to set deep-sleep timer)")
            return False
        ...
        if not is_ok(deep_resp):
            log_error("TCID12_WLANExternalWake Failed ❌ (failed to enter DEEP_SLEEP)")
            return False

        time.sleep(3)
        if not _post_deepsleep("DeepSleep_Wakeup_WLAN.yaml"):
            log_error("TCID12_WLANExternalWake Failed ❌ (failed to post WLAN wake simulation)")
            return False

        state = _wait_for_awake_state()
        if not isinstance(state, dict):
            log_error("TCID12_WLANExternalWake Failed ❌ (device did not report a post-wake power state)")
            return False

        reason = _wait_for_wakeup_reason("WIFI")
        if reason != "WIFI":
            log_error("TCID12_WLANExternalWake Failed ❌ (last wakeup reason was not WIFI)")
            return False
        ...
        if keycode != 0:
            log_error("TCID12_WLANExternalWake Failed ❌ (WLAN wake should not report a non-zero keycode)")
            return False
        ...
        if mode is not True:
            log_error("TCID12_WLANExternalWake Failed ❌ (network standby not enabled after WLAN wake)")
            return False
        ...
        if not isinstance(post_config, list):
            log_error("TCID12_WLANExternalWake Failed ❌ (invalid wakeup config after WLAN wake)")
            return False
        if get_source_enabled(post_config, "WIFI") is not True or get_source_enabled(post_config, "LAN") is not True:
            log_error("TCID12_WLANExternalWake Failed ❌ (WIFI/LAN not enabled after WLAN wake)")
            return False
        if get_source_enabled(post_config, "TIMER") is not True:
            log_error("TCID12_WLANExternalWake Failed ❌ (TIMER not enabled after WLAN wake)")
            return False
    finally:
        restore_entries = [
    ...
    msg = "TCID12_WLANExternalWake Passed ✅"
