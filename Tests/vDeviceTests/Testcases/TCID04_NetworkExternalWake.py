"""
/**
 * @file TCID04_NetworkExternalWake.py
 * @brief L3 PowerManager combination testcase.
 *
 * @testcase TCID04_NetworkExternalWake
...
def run_test():
    ...
    if not isinstance(original_mode, bool):
        log_error("TCID04_NetworkExternalWake Failed ❌ (unable to read baseline network standby mode)")
        return False
    ...
    if not isinstance(original_config, list):
        log_error("TCID04_NetworkExternalWake Failed ❌ (unable to read baseline config)")
        return False
    ...
    try:
        ...
        if not is_ok(nw_register_resp):
            log_error("TCID04_NetworkExternalWake Failed ❌ (network-standby event registration failed)")
            return False

        enable_resp = _set_network_standby(True)
        log_warning(f"Enable response: {enable_resp}")
        if not is_ok(enable_resp):
            log_error("TCID04_NetworkExternalWake Failed ❌ (failed to enable network standby)")
            return False

        if not note_partial("Async onNetworkStandbyModeChanged delivery is not asserted by this framework."):
            log_error("TCID04_NetworkExternalWake Failed ❌ (strict partial mode enabled)")
            return False
        ...
        if not is_ok(timer_enable_resp):
            log_error("TCID04_NetworkExternalWake Failed ❌ (failed to enable timer-backed network wake configuration)")
            return False
        ...
        if not isinstance(config, list):
            log_error("TCID04_NetworkExternalWake Failed ❌ (invalid wakeup config)")
            return False
        if get_source_enabled(config, "WIFI") is not True or get_source_enabled(config, "LAN") is not True:
            log_error("TCID04_NetworkExternalWake Failed ❌ (WIFI/LAN not enabled before network wake test)")
            return False

        if get_source_enabled(config, "TIMER") is not True:
            log_error("TCID04_NetworkExternalWake Failed ❌ (TIMER must already be enabled before deep sleep)")
            return False
        ...
        if not is_ok(timer_resp):
            log_error("TCID04_NetworkExternalWake Failed ❌ (failed to set deep-sleep timer)")
            return False
        ...
        if not is_ok(deep_resp):
            log_error("TCID04_NetworkExternalWake Failed ❌ (failed to enter DEEP_SLEEP)")
            return False

        time.sleep(3)
        if not _post_deepsleep("DeepSleep_Wakeup_LAN.yaml"):
            log_error("TCID04_NetworkExternalWake Failed ❌ (failed to post LAN wake simulation)")
            return False

        state = _wait_for_awake_state()
        if not isinstance(state, dict):
            log_error("TCID04_NetworkExternalWake Failed ❌ (device did not report a post-wake power state)")
            return False

        reason = _wait_for_wakeup_reason("LAN")
        if reason != "LAN":
            log_error("TCID04_NetworkExternalWake Failed ❌ (last wakeup reason was not LAN)")
            return False
        ...
        if keycode != 0:
            log_error("TCID04_NetworkExternalWake Failed ❌ (network wake should not report a non-zero keycode)")
            return False
        ...
        if mode is not True:
            log_error("TCID04_NetworkExternalWake Failed ❌ (network standby not enabled after LAN wake)")
            return False
        ...
        if not isinstance(post_config, list):
            log_error("TCID04_NetworkExternalWake Failed ❌ (invalid wakeup config after LAN wake)")
            return False
        if get_source_enabled(post_config, "WIFI") is not True or get_source_enabled(post_config, "LAN") is not True:
            log_error("TCID04_NetworkExternalWake Failed ❌ (WIFI/LAN not enabled after LAN wake)")
            return False
    finally:
        restore_entries = [
    ...
    msg = "TCID04_NetworkExternalWake Passed ✅"
