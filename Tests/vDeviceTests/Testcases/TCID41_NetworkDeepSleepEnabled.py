"""
/**
 * @file TCID41_NetworkDeepSleepEnabled.py
 * @brief L2 PowerManager combination testcase.
 *
 * @testcase TCID41_NetworkDeepSleepEnabled
...
from PowerManager_CombinationHelpers import get_source_enabled, note_partial, parse_network_standby, parse_power_state, parse_wakeup_config, wakeup_map
...
def run_test():
    ...
    if not isinstance(original_mode, bool):
        log_error("TCID41_NetworkDeepSleepEnabled Failed ❌ (unable to read baseline network standby mode)")
        return False
    ...
    if not isinstance(original_config, list):
        log_error("TCID41_NetworkDeepSleepEnabled Failed ❌ (unable to read baseline config)")
        return False
    ...
    try:
        ...
        if not is_ok(enable_resp):
            log_error("TCID41_NetworkDeepSleepEnabled Failed ❌ (failed to enable network standby)")
            return False
        ...
        if not isinstance(enabled_config, list):
            log_error("TCID41_NetworkDeepSleepEnabled Failed ❌ (invalid enabled config)")
            return False
        if get_source_enabled(enabled_config, "WIFI") is not True or get_source_enabled(enabled_config, "LAN") is not True:
            log_error("TCID41_NetworkDeepSleepEnabled Failed ❌ (WIFI/LAN not enabled before deep sleep)")
            return False
        ...
        if not is_ok(register_resp):
            log_error("TCID41_NetworkDeepSleepEnabled Failed ❌ (deep-sleep-timeout event registration failed)")
            return False

        if not note_partial("Async onDeepSleepTimeout delivery is not asserted by this framework."):
            log_error("TCID41_NetworkDeepSleepEnabled Failed ❌ (strict partial mode enabled)")
            return False
        ...
        if not is_ok(timer_resp):
            log_error("TCID41_NetworkDeepSleepEnabled Failed ❌ (failed to set deep-sleep timer)")
            return False
        ...
        if not is_ok(deep_resp):
            log_error("TCID41_NetworkDeepSleepEnabled Failed ❌ (deep-sleep request failed)")
            return False
        ...
        if not isinstance(state, dict) or state.get("currentState") != "LIGHT_SLEEP":
            log_error("TCID41_NetworkDeepSleepEnabled Failed ❌ (expected LIGHT_SLEEP after wake)")
            return False
        ...
        if mode is not True:
            log_error("TCID41_NetworkDeepSleepEnabled Failed ❌ (network standby not enabled after cycle)")
            return False
        ...
        if not isinstance(config, list):
            log_error("TCID41_NetworkDeepSleepEnabled Failed ❌ (invalid wakeup config after cycle)")
            return False
        if get_source_enabled(config, "WIFI") is not True or get_source_enabled(config, "LAN") is not True:
            log_error("TCID41_NetworkDeepSleepEnabled Failed ❌ (WIFI/LAN not enabled after cycle)")
            return False
    finally:
        restore_entries = [
    ...
    msg = "TCID41_NetworkDeepSleepEnabled Passed ✅"
