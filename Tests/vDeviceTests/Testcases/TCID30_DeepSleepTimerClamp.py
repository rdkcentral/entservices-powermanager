"""
/**
 * @file TCID30_DeepSleepTimerClamp.py
 * @brief L2 PowerManager combination testcase.
 *
 * @testcase TCID30_DeepSleepTimerClamp
...
def run_test():
    ...
    if not is_ok(large_resp):
        log_error("TCID30_DeepSleepTimerClamp Failed ❌ (oversized timer request failed)")
        return False
    ...
    if not is_ok(followup_resp):
        log_error("TCID30_DeepSleepTimerClamp Failed ❌ (follow-up timer request failed)")
        return False
    ...
    if not note_partial("The >86400 clamp exists in code, but runtime behavior after normalization is backend-dependent and is not asserted here."):
        log_error("TCID30_DeepSleepTimerClamp Failed ❌ (strict partial mode enabled)")
        return False

    elapsed_time = time.perf_counter() - start_time
    msg = "TCID30_DeepSleepTimerClamp Passed ✅"
