"""
/**
 * @file TCID24_NetworkStandbyToggle.py
 * @brief L2 PowerManager combination testcase.
 *
 * @testcase TCID24_NetworkStandbyToggle
...
def run_test():
    ...
    if not isinstance(original_mode, bool):
        log_error("TCID24_NetworkStandbyToggle Failed ❌ (unable to read baseline network standby mode)")
        return False
    ...
    if not isinstance(baseline_config, list):
        log_error("TCID24_NetworkStandbyToggle Failed ❌ (unable to read baseline config)")
        return False
    ...
    try:
        ...
        if not is_ok(enable_resp):
            log_error("TCID24_NetworkStandbyToggle Failed ❌ (enable request failed)")
            return False
        ...
        if not isinstance(enabled_config, list):
            log_error("TCID24_NetworkStandbyToggle Failed ❌ (invalid enabled config)")
            return False
        ...
        if enabled_mode is not True:
            log_error("TCID24_NetworkStandbyToggle Failed ❌ (network standby not enabled after enable request)")
            return False

        disable_resp = _set_network_standby(False)
        log_warning(f"Disable response: {disable_resp}")
        if not is_ok(disable_resp):
            log_error("TCID24_NetworkStandbyToggle Failed ❌ (disable request failed)")
            return False
        ...
        if not isinstance(disabled_config, list):
            log_error("TCID24_NetworkStandbyToggle Failed ❌ (invalid disabled config)")
            return False
        ...
        if disabled_mode is not False:
            log_error("TCID24_NetworkStandbyToggle Failed ❌ (network standby not disabled after disable request)")
            return False

        if get_source_enabled(enabled_config, "WIFI") is not True or get_source_enabled(enabled_config, "LAN") is not True:
            log_error("TCID24_NetworkStandbyToggle Failed ❌ (enable flow did not enable WIFI/LAN)")
            return False

        if get_source_enabled(disabled_config, "WIFI") is not False or get_source_enabled(disabled_config, "LAN") is not False:
            log_error("TCID24_NetworkStandbyToggle Failed ❌ (disable flow did not disable WIFI/LAN)")
            return False
        ...
        if non_network_map(enabled_config) != non_network_map(baseline_config) or non_network_map(disabled_config) != non_network_map(baseline_config):
            log_error("TCID24_NetworkStandbyToggle Failed ❌ (non-network wake sources changed unexpectedly)")
            return False
    finally:
        _set_network_standby(original_mode)

    elapsed_time = time.perf_counter() - start_time
    msg = "TCID24_NetworkStandbyToggle Passed ✅"
