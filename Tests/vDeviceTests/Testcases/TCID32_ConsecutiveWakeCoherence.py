"""
/**
 * @file TCID32_ConsecutiveWakeCoherence.py
 * @brief L2 PowerManager combination testcase.
 *
 * @testcase TCID32_ConsecutiveWakeCoherence
...
def run_test():
    ...
    try:
        ...
        if not is_ok(first_timer_resp):
            log_error("TCID32_ConsecutiveWakeCoherence Failed ❌ (failed to set first deep-sleep timer)")
            return False
        ...
        if not is_ok(first_deep_resp):
            log_error("TCID32_ConsecutiveWakeCoherence Failed ❌ (first deep-sleep request failed)")
            return False
        ...
        if not isinstance(first_reason, str) or not first_reason:
            log_error("TCID32_ConsecutiveWakeCoherence Failed ❌ (invalid first wakeup reason)")
            return False

        first_key_resp = send_curl_command(PowerManagerApis.get_last_wakeup_key_code)
        log_warning(f"First wakeup keycode response: {first_key_resp}")
        first_key = parse_last_wakeup_keycode(first_key_resp)
        if first_key != 0:
            log_error("TCID32_ConsecutiveWakeCoherence Failed ❌ (expected first wakeup keycode 0)")
            return False
        ...
        if not is_ok(second_timer_resp):
            log_error("TCID32_ConsecutiveWakeCoherence Failed ❌ (failed to set second deep-sleep timer)")
            return False
        ...
        if not is_ok(second_deep_resp):
            log_error("TCID32_ConsecutiveWakeCoherence Failed ❌ (second deep-sleep request failed)")
            return False
        ...
        if not isinstance(second_reason, str) or not second_reason:
            log_error("TCID32_ConsecutiveWakeCoherence Failed ❌ (invalid second wakeup reason)")
            return False

        second_key_resp = send_curl_command(PowerManagerApis.get_last_wakeup_key_code)
        log_warning(f"Second wakeup keycode response: {second_key_resp}")
        second_key = parse_last_wakeup_keycode(second_key_resp)
        if second_key != 0:
            log_error("TCID32_ConsecutiveWakeCoherence Failed ❌ (expected second wakeup keycode 0)")
            return False
    finally:
        send_curl_command(PowerManagerApis.set_power_state("ON", standby_reason="PM-PLUGIN-016-restore"))

    elapsed_time = time.perf_counter() - start_time
    msg = "TCID32_ConsecutiveWakeCoherence Passed ✅"
