"""
/**
 * @file TCID28_NetworkStandbyRepeat.py
 * @brief L2 PowerManager combination testcase.
 *
 * @testcase TCID28_NetworkStandbyRepeat
 * @details Validates idempotent repeated enable and disable operations for
 *          network standby, including a third cycle for longer repeat
 *          stability.
 */
"""
...
def run_test():
    ...
    if not isinstance(original_mode, bool):
        log_error("TCID28_NetworkStandbyReturnOn Failed ❌ (unable to read baseline network standby mode)")
        return False
    ...
    try:
        ...
        if not is_ok(first_enable_resp):
            log_error("TCID28_NetworkStandbyRepeat Failed ❌ (first enable request failed)")
            return False
        ...
        if not is_ok(second_enable_resp):
            log_error("TCID28_NetworkStandbyRepeat Failed ❌ (second enable request failed)")
            return False

        third_enable_resp = _set_network_standby(True)
        log_warning(f"Third enable response: {third_enable_resp}")
        if not is_ok(third_enable_resp):
            log_error("TCID28_NetworkStandbyRepeat Failed ❌ (third enable request failed)")
            return False
        ...
        if not isinstance(enabled_config, list):
            log_error("TCID28_NetworkStandbyRepeat Failed ❌ (invalid enabled config)")
            return False
        ...
        if enabled_mode is not True:
            log_error("TCID28_NetworkStandbyRepeat Failed ❌ (network standby not enabled after repeated enable)")
            return False
        if get_source_enabled(enabled_config, "WIFI") is not True or get_source_enabled(enabled_config, "LAN") is not True:
            log_error("TCID28_NetworkStandbyRepeat Failed ❌ (WIFI/LAN not enabled after repeated enable)")
            return False
        ...
        if not is_ok(first_disable_resp):
            log_error("TCID28_NetworkStandbyRepeat Failed ❌ (first disable request failed)")
            return False
        ...
        if not is_ok(second_disable_resp):
            log_error("TCID28_NetworkStandbyRepeat Failed ❌ (second disable request failed)")
            return False

        third_disable_resp = _set_network_standby(False)
        log_warning(f"Third disable response: {third_disable_resp}")
        if not is_ok(third_disable_resp):
            log_error("TCID28_NetworkStandbyRepeat Failed ❌ (third disable request failed)")
            return False
        ...
        if not isinstance(disabled_config, list):
            log_error("TCID28_NetworkStandbyRepeat Failed ❌ (invalid disabled config)")
            return False
        ...
        if disabled_mode is not False:
            log_error("TCID28_NetworkStandbyRepeat Failed ❌ (network standby not disabled after repeated disable)")
            return False
        if get_source_enabled(disabled_config, "WIFI") is not False or get_source_enabled(disabled_config, "LAN") is not False:
            log_error("TCID28_NetworkStandbyRepeat Failed ❌ (WIFI/LAN not disabled after repeated disable)")
            return False
    finally:
        _set_network_standby(original_mode)

    elapsed_time = time.perf_counter() - start_time
    msg = "TCID28_NetworkStandbyRepeat Passed ✅"
