"""
/**
 * @file TCID27_InvalidWakeupUpdate.py
 * @brief L2 PowerManager combination testcase.
 *
 * @testcase TCID27_InvalidWakeupUpdate
...
from PowerManager_CombinationHelpers import parse_error, parse_last_wakeup_reason
...
def run_test():
    ...
    if not is_ok(valid_resp):
        log_error("TCID27_InvalidWakeupUpdate Failed ❌ (failed to set valid wakeup config)")
        return False
    ...
    if not isinstance(parse_error(invalid_resp), dict):
        log_error("TCID27_InvalidWakeupUpdate Failed ❌ (invalid update was not rejected)")
        return False
    ...
    if not is_ok(timer_resp):
        log_error("TCID27_InvalidWakeupUpdate Failed ❌ (failed to set deep-sleep timer)")
        return False
    ...
    if not is_ok(deep_resp):
        log_error("TCID27_InvalidWakeupUpdate Failed ❌ (failed to enter DEEP_SLEEP)")
        return False
    ...
    if reason != "TIMER":
        log_error("TCID27_InvalidWakeupUpdate Failed ❌ (wake reason did not follow last valid config)")
    ...
    msg = "TCID27_InvalidWakeupUpdate Passed ✅"
