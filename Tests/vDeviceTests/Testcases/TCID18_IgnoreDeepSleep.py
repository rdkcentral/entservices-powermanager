"""
/**
 * @file TCID18_IgnoreDeepSleep.py
 * @brief L2 PowerManager combination testcase.
 *
 * @testcase TCID18_IgnoreDeepSleep
...
def run_test():
    ...
    if not can_manage_ignoredeepsleep_file():
        if not note_partial("Target-local Linux file access is required for /tmp/ignoredeepsleep handling."):
            log_error("TCID18_IgnoreDeepSleep Failed ❌ (strict partial mode enabled)")
            return False
        elapsed_time = time.perf_counter() - start_time
        msg = "TCID18_IgnoreDeepSleep Passed ✅"
    ...
    try:
        ...
        if not is_ok(on_resp):
            log_error("TCID18_IgnoreDeepSleep Failed ❌ (failed to set ON baseline)")
            return False
        ...
        if not isinstance(before_state, dict):
            log_error("TCID18_IgnoreDeepSleep Failed ❌ (invalid baseline state)")
            return False
        ...
        if not isinstance(after_state, dict) or after_state.get("currentState") != before_state.get("currentState"):
            log_error("TCID18_IgnoreDeepSleep Failed ❌ (state changed despite override)")
            return False

        if not isinstance(parse_error(deep_resp), dict):
            if not is_ok(deep_resp):
                log_error("TCID18_IgnoreDeepSleep Failed ❌ (unexpected DEEP_SLEEP response)")
                return False
