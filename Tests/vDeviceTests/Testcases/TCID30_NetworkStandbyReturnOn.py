"""
/**
 * @file TCID30_NetworkStandbyReturnOn.py
 * @brief L2 PowerManager combination testcase.
 *
 * @testcase TCID30_NetworkStandbyReturnOn
...
from PowerManager_CombinationHelpers import build_wakeup_override_entries, get_source_enabled, parse_network_standby, parse_power_state, parse_time_since_wakeup, parse_wakeup_config, wakeup_map
...
def run_test():
    ...
    if not isinstance(original_mode, bool):
        log_error("TCID30_NetworkStandbyReturnOn Failed ❌ (unable to read baseline network standby mode)")
        return False
    ...
    if not isinstance(original_config, list):
        log_error("TCID30_NetworkStandbyReturnOn Failed ❌ (unable to read baseline config)")
        return False
    ...
    try:
        ...
        if not is_ok(enable_resp):
            log_error("TCID30_NetworkStandbyReturnOn Failed ❌ (failed to enable network standby)")
            return False
        ...
        if not is_ok(timer_enable_resp):
            log_error("TCID30_NetworkStandbyReturnOn Failed ❌ (failed to enable TIMER wake source)")
            return False
        ...
        if not isinstance(timer_enabled_config, list):
            log_error("TCID30_NetworkStandbyReturnOn Failed ❌ (invalid timer-enabled config)")
            return False
        if get_source_enabled(timer_enabled_config, "TIMER") is not True:
            log_error("TCID30_NetworkStandbyReturnOn Failed ❌ (TIMER not enabled before deep sleep)")
            return False
        if get_source_enabled(timer_enabled_config, "WIFI") is not True or get_source_enabled(timer_enabled_config, "LAN") is not True:
            log_error("TCID30_NetworkStandbyReturnOn Failed ❌ (WIFI/LAN changed while enabling TIMER)")
            return False
        ...
        if not is_ok(timer_resp):
            log_error("TCID30_NetworkStandbyReturnOn Failed ❌ (failed to set deep-sleep timer)")
            return False
        ...
        if not is_ok(deep_resp):
            log_error("TCID30_NetworkStandbyReturnOn Failed ❌ (deep-sleep request failed)")
            return False
        ...
        if not is_ok(on_resp):
            log_error("TCID30_NetworkStandbyReturnOn Failed ❌ (failed to return to ON)")
            return False

        if not _wait_for_on_state(5):
            log_error("TCID30_NetworkStandbyReturnOn Failed ❌ (power state did not reach ON)")
            return False
        ...
        if not isinstance(wake_age, int) or wake_age <= 0:
            log_error("TCID30_NetworkStandbyReturnOn Failed ❌ (expected non-zero wakeup age after ON)")
            return False
        ...
        if mode is not True:
            log_error("TCID30_NetworkStandbyReturnOn Failed ❌ (network standby not enabled after cycle)")
            return False
        ...
        if not isinstance(config, list):
            log_error("TCID30_NetworkStandbyReturnOn Failed ❌ (invalid wakeup config after cycle)")
            return False
        if get_source_enabled(config, "WIFI") is not True or get_source_enabled(config, "LAN") is not True:
            log_error("TCID30_NetworkStandbyReturnOn Failed ❌ (WIFI/LAN not enabled after cycle)")
            return False
    finally:
        restore_entries = [
    ...
    msg = "TCID30_NetworkStandbyReturnOn Passed ✅"
