"""
/**
 * @file TCID31_DuplicateWakeupSource.py
 * @brief L2 PowerManager combination testcase.
 *
 * @testcase TCID31_DuplicateWakeupSource
...
def run_test():
    ...
    if not isinstance(original_config, list):
        log_error("TCID31_DuplicateWakeupSource Failed ❌ (unable to read baseline config)")
        return False
    ...
    if not is_ok(set_resp):
        log_error("TCID31_DuplicateWakeupSource Failed ❌ (duplicate payload request failed)")
        return False
    ...
    if not isinstance(read_config, list) or get_source_enabled(read_config, "IR") is not False:
        log_error("TCID31_DuplicateWakeupSource Failed ❌ (IR did not resolve to the last payload value)")
        return False
    ...
    msg = "TCID31_DuplicateWakeupSource Passed ✅"
