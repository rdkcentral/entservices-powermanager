"""
/**
 * @file TCID18_WakeupAgeReset.py
 * @brief L2 PowerManager combination testcase.
 *
 * @testcase TCID18_WakeupAgeReset
...
def run_test():
    ...
    if not is_ok(on_resp):
        log_error("TCID18_WakeupAgeReset Failed ❌ (failed to set ON baseline)")
        return False
    ...
    if not isinstance(first_value, int):
        log_error("TCID18_WakeupAgeReset Failed ❌ (invalid first wakeup age)")
        return False
    ...
    if not isinstance(second_value, int) or second_value < first_value:
        log_error("TCID18_WakeupAgeReset Failed ❌ (wakeup age not monotonic)")
        return False
    ...
    if not is_ok(standby_resp):
        log_error("TCID18_WakeupAgeReset Failed ❌ (failed to set STANDBY)")
    ...
    if reset_value != 0:
        log_error("TCID18_WakeupAgeReset Failed ❌ (wake age did not reset outside ON)")
        send_curl_command(PowerManagerApis.set_power_state("ON", standby_reason="PM-PLUGIN-002-restore"))
        return False
    ...
    msg = "TCID18_WakeupAgeReset Passed ✅"
